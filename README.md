# 📚 [roadmap.sh](https://roadmap.sh/devops/projects) 
## ♾️ [DevOps Projects](https://roadmap.sh/devops/projects)

| # | Projeto |
|---|--------|
| 1 | 📊 [Server Performance Stats](https://roadmap.sh/projects/server-stats) ✅ |
| 2 | 🗄️ Log Archive Tool |
| 3 | 📜 Nginx Log Analyser |
| 4 | 🚀 GitHub Pages Deployment |
| 5 | 🔐 SSH Remote Server Setup |
| 6 | 🌐 Static Site Server |
| 7 | 🌍 Basic DNS Setup |
| 8 | 📡 Simple Monitoring |
| 9 | ⚙️ Dummy Systemd Service |
|10 | 🐳 Basic Dockerfile |
|11 | ☁️ EC2 Instance |
|12 | ⏱️ Pomodoro Timer |
|13 | 🛠️ Configuration Management |
|14 | 🏗️ IaC on DigitalOcean |
|15 | 🟢 Node.js Service Deployment |
|16 | 📦 Dockerized Service |
|17 | 🧩 Multi-Container Application |
|18 | 💾 Automated DB Backups |
|19 | 🏰 Bastion Host |
|20 | 🔎 File Integrity Checker |
|21 | 🐧 Linux Server Setup |
|22 | 🔒 VPN Server Setup |
|23 | 🔄 Blue-Green Deployment |
|24 | 📈 Prometheus and Grafana |
|25 | 🧱 Multi-Service Application |
|26 | 🧭 Service Discovery |

Todos os projetos podem ser acessados em [roadmap.sh](https://roadmap.sh/devops/projects)
